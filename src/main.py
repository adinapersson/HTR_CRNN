...
def loadData(): # Returns information of samples split in train, validation and test sets
    lines_words = []
    words_path = os.path.join(base_path,"words.txt")
    words = open(words_path, "r",encoding="utf-8").readlines()
    # Exclude error segmentations from dataset
    for line in words:
        if line[0] == "#":
            continue
        if line.split(" ")[1] != "err":
    ...
    val_split_idx = int(0.5 * len(test_data)) # 5% validation data and 5% test data
...
def get_image_paths_and_labels(data): # Returns paths and labels for the data
    paths = []
    labels = []
    for file_line in enumerate(data):
        ...
        # partI/partI-partII/partI-partII-image_name.png
...
def get_max_length_and_chars(labels): # Returns maximum length and the vocabulary in the training data
    characters = set()
    max_length = 0

    for label in labels:
        for char in label:
            characters.add(char) # Add all characters present in labels to the vocab

        max_length = max(max_length, len(label)) # Find maximum length
    print("Maximum length: ", max_length)
    print("Vocab size: ", len(characters))
    return max_length, sorted(characters)


def preprocess_image(image_path): # Returns the image
...
def vectorize_label(label): # Returns label (padded to max_length)
...
def prepare_dataset(image_paths, labels, batch_size): # Prepares the dataset
...
def viewSamples(train_ds): # Plotting samples from the training data (images and labels)
...
def calculate_edit_distance(labels, predictions): # Calculates and returns edit distance
...
class EditDistanceCallback(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None): # Prints mean edit distance for each epoch
        edit_distances = []
...
def train_model(model,train_ds,validation_ds,epochs,model_name,toTrain,toLoad): # Trains the model and returns the prediction model
    ...
    # Get the prediction model by extracting layers to the output layer
...
def accuracy(validation_ds,prediction_model): # Calculates and returns the accuracy 
    accuracy = 0
    for batch in validation_ds:
...
def viewResults(test_ds, prediction_model): # Prints the result for some samples (images, predicted text and ground truth text)
...
def writeReport(acc,epochs,dropout,model_name,num_train): # Writing report for model
...
def main():
    ...
    # Print number of training, testing and validation data
    ...
    # Getting max_length and vocabulary
    max_length, characters = get_max_length_and_chars(train_labels)

    # Printing the vocabulary and number of characters in the vocab
    print(characters)
